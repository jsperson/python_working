SELECT
	...
	CAST(nvl(sum(mtm.exp_settlement_amt_contract),0)*(nvl(sum(inv.actual_quantity_32e),0)/decode(sum(mtm.curr_quantity_32e),0,1,null,1,sum(mtm.curr_quantity_32e))) AS NUMERIC(28,10)) AS total_settlement_amt_32e,
	...
	CAST(nvl(sum(mtm.exp_settlement_amt_contract),0)*(nvl(sum(inv.actual_quantity),0)/decode(sum(mtm.curr_quantity_contract),0,1,null,1,sum(mtm.curr_quantity_contract))) AS NUMERIC(28,10)) AS total_settlement_amt_contract_uom,
